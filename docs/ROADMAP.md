...
**Target.** Prove that coefficient-safe parity words cannot concentrate on
unusually small positive residues under the inverse parity map.

**Why this would solve Collatz.** A uniform small-residue exclusion gives the
needed lower bound for `M(k)` directly.

**Missing theorem.** A deterministic anti-concentration statement over the
generalized Ballot language, not an average over all parity words.

**Fast falsification test.** Measure the smallest residues in exact dynamic
programming/meet-in-the-middle slices and isolate words responsible for every
record drop; test proposed tail bounds against those records.
